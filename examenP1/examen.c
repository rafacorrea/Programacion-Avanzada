...
void agregarPersona(Persona **, int*);//funcion para agregarpersonas
void verPersonas(Persona * , int );//funcion para ver el arreglo
void editarPersona(Persona ** p, int* personas);//funcion para editar
void borrarPersona(Persona ** , int , int * );//funcion para borrar del arreglo
void borrar(Persona **p, int * personas);//funcion menu para llamar a borrarPersona
void crearEdificacion(Edificacion ** e, int * edificaciones);//crear modelos
void verMisModelos(Persona * p, Edificacion * e, int personas, int edificaciones);//ver modelos de una persona
void reporte(Persona * p, Edificacion * e, int personas, int edificaciones);//generar el reporte de recursos humanos

void freeAll(Persona * p,  Edificacion * e, int personas, int edificaciones);
...
main()
{
    ...
    freeAll(p, e, personas, edificaciones);
    return 0;
}
...
void freeAll(Persona * p,  Edificacion * e, int personas, int edificaciones)
{
    ...
    /*for(i=0; i<edificaciones;i++)
    {
        free((e+i)->modales);
        free((e+i)->dependeTipo);
        free((e+i)->fecha);
...
void crearEdificacion(Edificacion ** e, int * edificaciones)
{
    ...
    *edificaciones+=1;
	...
	switch(rand()%3)
    {
		//para que se vea que en el reporte si ordena por fecha..
		case 0:
	...
	printf("Ingrese la nomina de la persona que hizo este modelo: ");
    scanf("%d",&((aux)->autorNomina));
    switch (temptipo)
    {
        case 1:
        {
...
void verMisModelos(Persona * p, Edificacion * e, int personas, int edificaciones)
{
    ...
    if(fork()==0) //para simular un trabajador activo
    {
		for( ; e < final; e++)
		{
			...
			if(e->autorNomina == nomina && !strcmp(e->fecha, fecha)) //si la fecha coincide
			{
		             char * tipo;
		             if(e->tipo == 1)
		                 tipo = "Edificio";
...
void reporte(Persona * p, Edificacion * e, int personas, int edificaciones)
{
	...
	/*modPorFecha = inicioF;
	finalF = inicioF + cuantosDias;
	for( ;modPorFecha<finalF; modPorFecha++)
	{
		free(modPorFecha->fecha);
	}
	free(modPorFecha);*/
}
